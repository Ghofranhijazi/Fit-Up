...
const NurseryProfile = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/api/users/${userId}`, { withCredentials: true })
      .then((res) => setNursery(res.data))
      .catch((err) => console.error(err));

    // Fetch nursery bookings instead of gym bookings
    axios.get(`http://localhost:5000/api/nurseries/${userId}/bookings`, { withCredentials: true })
      .then((res) => setBookings(res.data))
      .catch((err) => console.error('Error fetching bookings:', err));
  }, [userId]);

  const handleChange = (e) => setNursery({ ...nursery, [e.target.name]: e.target.value });
  ...
  const handleSave = async () => {
    const formData = new FormData();
    formData.append('phone', nursery.phone);
    formData.append('address', nursery.address);
    formData.append('username', nursery.username);
    formData.append('email', nursery.email);
    formData.append('role', nursery.role);
    // if (nursery.profileImage instanceof File) {
    //   formData.append('profileImage', nursery.profileImage);
    // }
  ...
  if (!nursery.role) return <div>Loading...</div>;
  if (nursery.role !== 'nurseryOwner') return <div>Unauthorized</div>;
      ...
      <div className="mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 sm:gap-0">
          <div className="relative mb-4 sm:mb-0">
            ...
            >
              {/* {nursery?.profileImage ? (
                <img
                  src={nursery.profileImage instanceof File ? 
                    URL.createObjectURL(nursery.profileImage) : 
                    `http://localhost:5000${nursery.profileImage}`}
                  alt="Profile"
          ...
          <div className="sm:ml-4 md:ml-6 text-center sm:text-left flex-1 min-w-0">
            ...
            >
              {nursery?.username || 'Nursery Owner'}
            </h1>
            <p className="text-sm sm:text-base text-gray-600 truncate">{nursery?.email}</p>
          </div>
              ...
              <div className="bg-white rounded-xl shadow-xl w-full max-w-xs sm:max-w-sm md:max-w-md mx-2 p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div className="border-b pb-3 sm:pb-4">
                    ...
                    <p className="font-semibold text-sm sm:text-base">{selectedBooking.nursery?.nurseryName || 'Nursery Name Not Available'}</p>
                    <p className="text-gray-600 text-xs sm:text-sm">{selectedBooking.nursery?.address || 'Address Not Available'}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-sm sm:text-base">{selectedBooking.selectedPlan} JD</p>
                      </div>
                    </div>
                  </div>
                </div>
